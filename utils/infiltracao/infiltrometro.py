...
class Infiltrometro:
    ...
    def __init__(self, infiltrations:gpd.GeoDataFrame|pd.DataFrame):
        self.infiltrations = infiltrations.copy(        )

        m1 = ~self.infiltrations["Clay"].isna()
        m2 = ~self.infiltrations["Silt"].isna()
        m3 = ~self.infiltrations["Sand"].isna()

        mask = m1 & m2 & m3

        self.infiltrations = self.infiltrations[mask]
        self.infiltrations.reset_index(inplace=True, drop=True)
    ...
    def _calculate_C1_C2(self):
        self.infiltrations["COV_C1_C2"] = None

        columns_time = np.array(COLUMNS_INFILTRATION[4:25])
        ...
        for ponto in self.infiltrations.itertuples():
            ...
            I_data = np.array(pd.to_numeric(ponto[5:26], errors="coerce"))
    ...
    def A2(self, point:str|None = None):
        ...
        `Retorna o valor None em A Dohnal caso ele não seja válido`
        """
