...
from app.fastapi_types import UserType, ProjectType, FlowType, NodeType, EdgeType
...
async def get_user(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    studies = db.query(Project).filter(Project.user == current_user).all()
    return [i.to_json() for i in studies]
...
@project_route.put("", response_model=UserType)
async def update_user(
    user_data: UserType,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
